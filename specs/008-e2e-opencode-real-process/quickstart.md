...
E2E-тесты используют тестовый агент `e2e-responder` для обработки запросов.

### Конфигурация

| Параметр | Значение |
|----------|-----------|
| Файл | `.opencode/agents/e2e-responder.md` |
| Назначение | Тестовый агент без инструментов |
| Поведение | Краткие текстовые ответы |

### Инструкции агента

Агент `e2e-responder` — это минималистичный агент для E2E-тестирования:

1. Отвечает максимально кратко (одно предложение или несколько слов)
2. Не использует инструменты (file read, bash, web search и т.д.)
3. Отвечает на любые вопросы, включая абсурдные
4. Математические вопросы — точный числовой ответ
5. Фактологические вопросы — короткий ответ
...
Агент `e2e-responder` автоматически подключается при запуске `opencode serve :3001` в E2E-тестах. Конфигурация передаётся через параметр `--agents` или загружается из `.opencode/agents/e2e-responder.md`.
```